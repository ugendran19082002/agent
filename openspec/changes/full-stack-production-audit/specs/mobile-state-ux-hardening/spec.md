...
- **THEN** it displays a reusable loading indicator or skeleton instead of a blank view

#### Scenario: Screen has no records
- **WHEN** an API returns an empty result set
- **THEN** the screen displays a reusable empty state with a relevant next action when one exists
...
Every async button action SHALL prevent duplicate submission and expose clear pending, success, validation error, API error, and network failure feedback.

#### Scenario: Login button is pressed
- **WHEN** the user presses login with valid input
- **THEN** the button enters a submitting state, duplicate presses are ignored, and the next auth step or error message is shown

#### Scenario: Payment button is pressed repeatedly
- **WHEN** the user taps payment multiple times while a payment attempt is pending
- **THEN** only one payment attempt is created and the UI remains responsive with visible progress
