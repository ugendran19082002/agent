...
- [x] 23.2 Remove the `/api/...` deprecation shim from `routes/index.routes.js` once analytics confirm < 1% of traffic still hits the unversioned paths
- [x] 23.3 Rename `user_can_balance` → `user_can_balance_archive` after a 2-week soak window with no production reads against the legacy table
